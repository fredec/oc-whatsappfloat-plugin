...
class SendMail {
    ...
    public static function sendNotification($properties, $post, $record, $files) {
        ...
        if (is_array($properties['mail_recipients']) || is_array($properties['mail_bcc'])) {
            ...
            // CUSTOM TEMPLATE
            $template = isset($properties['mail_template']) && $properties['mail_template'] != '' && MailTemplate::where('code', $properties['mail_template'])->count() ? $properties['mail_template'] : 'diveramkt.whatsappfloat::mail.notification';
    ...
    public static function sendAutoResponse($properties, $post, $record) {
        ...
        if (filter_var($to, FILTER_VALIDATE_EMAIL) && filter_var($from, FILTER_VALIDATE_EMAIL)) {
            ...
            // CUSTOM TEMPLATE
            $template = isset($properties['mail_resp_template']) && $properties['mail_resp_template'] != '' && MailTemplate::where('code', $properties['mail_resp_template'])->count() ? $properties['mail_resp_template'] : 'diveramkt.whatsappfloat::mail.autoresponse';
